...
const Search = ({ match }) => {
  ...
  if (loading) {
    return loading ? (
      <div className="async">
        <img src ="https://miro.medium.com/max/675/1*2itj3a68Yg76E8OfOe2O6A.gif" className="loading" alt = "Loading..." />
        <h3>Loading</h3>
      </div>
    ) : null;
  }
      ...
      <div className="main_result_space">
        ...
        <div className="book_result">
          ...
          {match.params.page == 1 ? null : (
            <div className="table_btn"><button onClick={prev} className="Back">
              <h1>{'<<'} 이전</h1>
            </button></div>
          )}
          {books.length !== 0 ? (
            <div className="table_btn"><button onClick={next} className="Next">
              <h1>다음 {'>>'}</h1>
            </button></div>
          ) : null}
      </div>
